...
def align(images_path, dataset, results_path):
    ...
    template = cv2.imread(image_files[0])
    cv2.imwrite(os.path.join(results_path, dataset, image_files[0].split('/')[-1]), template)

    for f in image_files[1:]:
        template = image_alignment(template, cv2.imread(f))
        cv2.imwrite(os.path.join(results_path, dataset, f.split('/')[-1]), template)
